        ...
        public static string GetReward(Ei.Contract.Types.Goal goal) {
            switch(goal.RewardType) {
                ...
                case Ei.RewardType.EpicResearchItem:
                    var researchItem = (JsonData.EiEpicResearch.Get().epicResearchItems.AsQueryable().FirstOrDefault(x => x.id.Equals(goal.RewardSubType, StringComparison.CurrentCultureIgnoreCase)));
