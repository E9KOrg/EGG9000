...
namespace EGG9000.Bot.EggIncAPI {
    public class Research {
        ...
        public static double GetEggLayingRatePerSec(CustomFarm farm, List<CustomResearch> epic) {
            ...
            var researchItems = Research.EpicResearchList.Where(x => x.Type == Research.IT.EggLayingRate || x.Type == Research.IT.EggLayingAndValue).ToList();
            researchItems.AddRange(Research.CommonResearchList.Where(x => x.Type == Research.IT.EggLayingRate || x.Type == Research.IT.EggLayingAndValue));

            var eggRates = researchItems.Select(x => new {
                Rate = (x.Epic ?
                            epic.First(y => y.Id == x.id).Level :
                            farm.CommonResearch.First(y => y.Id == x.id).Level
                        ) * (double)x.Increase
            });

            foreach(var item in Research.EpicResearchList.Where(x => x.Type == Research.IT.EggLayingRate || x.Type == Research.IT.EggLayingAndValue)) {
            ...
            foreach(var item in Research.CommonResearchList.Where(x => x.Type == Research.IT.EggLayingRate || x.Type == Research.IT.EggLayingAndValue)) {
        ...
        public static double GetShippingCapacityPerSec(CustomFarm farm, List<CustomResearch> epic) {
            ...
            foreach(var item in Research.EpicResearchList.Where(x => x.Type == Research.IT.VehicleCapacity)) {
                var current = epic.First(x => x.Id == item.id);
                var r = current.Level * (double)item.Increase;
                if(r > 0) {
                    baseMultiplier *= 1 + r;
            ...
            foreach(var item in Research.CommonResearchList.Where(x => x.Type == Research.IT.VehicleCapacity)) {
                var current = farm.CommonResearch.First(x => x.Id == item.id);
                var r = current.Level * (double)item.Increase;
                if(r > 0) {
                    baseMultiplier *= 1 + r;
            ...
            foreach(var item in Research.CommonResearchList.Where(x => x.Type == Research.IT.HoverVehicleCapacity)) {
                var current = farm.CommonResearch.First(x => x.Id == item.id);
                var r = current.Level * (double)item.Increase;
                if(r > 0) {
                    hoverMultiplier *= 1 + r;
            ...
            foreach(var item in Research.CommonResearchList.Where(x => x.Type == Research.IT.HyperloopCapacity)) {
                var current = farm.CommonResearch.First(x => x.Id == item.id);
                var r = current.Level * (double)item.Increase;
                if(r > 0) {
                    hyperLoopMultipler *= 1 + r;
        ...
        public static Dictionary<uint, uint> Vehicles {
            get {
                var v = new Dictionary<uint, uint>();
                v.Add(0, 5000);
                v.Add(1, 15000);
                v.Add(2, 50000);
                v.Add(3, 100000);
