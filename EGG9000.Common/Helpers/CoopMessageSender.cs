...
namespace EGG9000.Common.Helpers {
    ...
    public class CoopMessageSender(ICoopCreationQueue queue, IServiceProvider provider, ILogger<CoopMessageSender> logger) {
        ...
        private int _laneCounter = -1;

        internal bool NextLaneIsWebhook() => Interlocked.Increment(ref _laneCounter) % 2 == 1;
        ...
        public async Task<TrackedMessage> SendAsync(IThreadChannel thread, HeaderChannelWebhook webhookInfo, string content = null, Embed embed = null) {
            if(webhookInfo != null && NextLaneIsWebhook()) {
                try {
        ...
        public async Task<TrackedMessage> EditAsync(IThreadChannel thread, TrackedMessage tracked, HeaderChannelWebhook webhookInfo, string content = null, Embed embed = null) {
            if(tracked.WebhookId != null) {
                if(webhookInfo == null || webhookInfo.WebhookId != tracked.WebhookId) {
                    ...
                    return await ReplaceStaleMessageAsync(thread, tracked, webhookInfo, content, embed);
                }
                try {
        ...
        // delete the stale message, then post a fresh replacement via SendAsync so future edits of
        // this coop's status message target a live lane instead of repeating the same failure forever.
        private async Task<TrackedMessage> ReplaceStaleMessageAsync(IThreadChannel thread, TrackedMessage tracked, HeaderChannelWebhook webhookInfo, string content, Embed embed) {
            try {
            ...
            return await SendAsync(thread, webhookInfo, content, embed);
        }
