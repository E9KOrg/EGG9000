...
namespace EGG9000.Bot.Commands {
    ...
    public class ApiVersionModule(IDbContextFactory<ApplicationDbContext> dbFactory, IServiceProvider provider) : E9KModuleBase(dbFactory) {
        ...
        public async Task SetVersions(
            ...
            [Summary(description: "App build string (e.g. 1.35.6.3)")] string appBuild) {

            await Context.Interaction.DeferAsync(ephemeral: true);

            if(clientVersion <= 0 || string.IsNullOrWhiteSpace(appVersion) || string.IsNullOrWhiteSpace(appBuild)) {
                await Context.Interaction.ModifyOriginalResponseAsync(x => { x.Content = ""; x.Embed = EmbedError("clientVersion must be positive and appVersion/appBuild must be non-empty."); });
                return;
            }

            var valid = await EggIncApi.ValidateVersionsAsync((uint)clientVersion, appVersion, appBuild);
            if(!valid) {
                await Context.Interaction.ModifyOriginalResponseAsync(x => { x.Content = ""; x.Embed = EmbedError($"Rejected `{clientVersion} / {appVersion} / {appBuild}`: the API returned no contracts (bad/stale/typo'd version, or a transient failure). Nothing changed."); });
                return;
            }
