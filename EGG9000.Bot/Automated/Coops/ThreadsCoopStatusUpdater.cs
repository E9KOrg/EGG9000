    ...
    public class ThreadsCoopStatusUpdater(IServiceProvider provider) : _UpdaterBase<ThreadsCoopStatusUpdater>(interval, delay, provider) {
        ...
        public async Task<bool> ProcessCoop(Guid coopId, SocketGuild guild, SocketGuild parentGuild, List<UserWithBackup> users, Guild dbGuild, CancellationToken cancellationToken) {
            ...
            try {
                ...
                //        var r = await EggIncApi.Post<Ei.ContractPlayerInfo, Ei.BasicRequestInfo>(new Ei.BasicRequestInfo(), account.EggIncId);
                ...
                if(usersNotJoined.Count == 0 && coop.Status != CoopStatusEnum.Completed && coop.Status != CoopStatusEnum.Failed && coop.Status != CoopStatusEnum.CompletedAllCheckIn) {
                    ...
                    foreach(var userFarmDetails in usersNotJoined) {
                        try {
                            ...
                            if(discordUser != null && !coop.Finished && coop.Status != CoopStatusEnum.Failed && coop.CoopEnds > DateTimeOffset.UtcNow) {
                                ...
                                if(userFarmDetails.Xref.CreatedOn < DateTimeOffset.UtcNow.AddHours(-hoursToKick)) {
...
;
                ...
                if(!coop.FinalizedFinishedOrFailed() || finalChannelUpdate) {
                    ...
                    if(coop.Status == CoopStatusEnum.Failed) {
                        ...
                        if(missingCount > 0) {
                            if(DateTimeOffset.UtcNow > (coop.Created + TimeSpan.FromHours(12))) {
                                if(missingCount <= 20) {
                    ...
                    try {
                        ...
                        coop.ProjectedFinish = DateTimeOffset.UtcNow.AddYears(1);
                    }
                ...
                coop.LastUpdateToChannel = DateTimeOffset.UtcNow;
                await _db.SaveChangesAsyncRetry(cancellationToken: CancellationToken.None, logger: _logger);
        ...
        private async Task<StatusResponse> GetStatus(Coop coop, ITextChannel channel, CancellationToken cancellationToken) {
            ...
            if(!coop.UserCoopsXrefs.Any(x => x.JoinedCoop)) {
                ...
                //statusTask = policy.Execute(async () => await EggIncApi.GetCoopStatus(coop.ContractID, coop.Name, EIID: coop.CreatorID, _logger: _logger, cancellationToken: cancellationToken));
