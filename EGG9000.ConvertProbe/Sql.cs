...
namespace EGG9000.ConvertProbe {
    public static class Sql {
        ...
        public static string ColumnExists(string table, string column) {
            return $"SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = '{table.Trim('"')}' AND column_name = '{column.Trim('"')}')";
        }
        ...
        public static string RelationSizes(string table) {
            return $"SELECT pg_total_relation_size('{table}')::bigint, pg_relation_size('{table}')::bigint";
        }

        public static string ColumnSize(string table, string column) {
            return $"SELECT coalesce(sum(pg_column_size({column})), 0)::bigint FROM {table}";
        }

        public static string ContractResponses {
            get {
                return $"SELECT {ContractsId}, {ResponseColumn} FROM {ContractsTable}";
            }
        }
