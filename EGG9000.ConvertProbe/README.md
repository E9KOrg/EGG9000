...
- `bench --limit 200`: decode/encode timings and sizes for legacy (`StorageMessagePack.Options`), envelope (current `StorageCompressionStrategy`), brotli q6 envelope, and master-equivalent (`Standard.WithCompression(Lz4BlockArray)`) encodes, plus compress/decompress-only timings on the plain bytes and an LZ4 block layout table (blocks per blob, distinct uncompressed block lengths) for stored, legacy, and master-equivalent bytes; writes `bench.md`.
- `traindict [--limit N]`: trains zstd dictionaries (110 KB) from N random accounts blobs (default 3000) and proportionally more coop status blobs, holding out 500 of each; writes `accounts.zdict`, `coopstatus.zdict`, and `traindict.md` (hold-out bytes and timings for plain, brotli q6, the current strategy with its embedded dictionary, zstd without dictionary, zstd with the new dictionary). Copy the `.zdict` files into `EGG9000.Common/Database/Dictionaries/` under a new name and register a new `StorageDictionary` id; never overwrite a registered dictionary.
